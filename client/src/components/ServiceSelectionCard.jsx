...
export default function ServiceSelectionCard({ service, selected, onToggle }) {
  return (
    <button type="button" onClick={() => onToggle(service)} className={`group relative rounded-2xl border bg-white p-5 text-left shadow-sm transition hover:-translate-y-1 hover:shadow-premium ${selected ? 'border-purple ring-4 ring-purple/10' : 'border-line'}`}>
